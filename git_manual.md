...
### *__Знакомство с удалёнными репозиториями__*

Для реализации большого проекта обычно группа разработчиков использует удаленный репозиторий. Веб-сервисов для размещения IT-проектов и их совместной разработки не так много. Один из них — [GitHub](https://github.com/).

>**Первое правило при использовании удаленного репозитория: каждый разработчик должен работать в отдельной ветке!**

Давайте рассмотрим некоторые команды, для работы которых требуется сетевое подключение.

```
<< git remote >>
```
Команда `git remote` служит для управления списком удаленных репозиториев. Флаг *` -v`* отображает адреса наших удалённых репозиториев.

![команда git remote -v](git_remote%20-v.PNG)

```
<< git fetch >>
```
Команда `git fetch` связывается с удаленным репозиторием и извлекает из него все изменения.

```
<< git pull >>
```
Команда `git pull` извлекает изменения из указанного удаленного репозитория, а затем пытается объединить их в текущую ветку. Он работает как комбинация команд `git fetch` и `git merge`.

```
<< git push >>
```
`git push` используется для установления соединения с удаленным репозиторием.
...
### *__Работа с веб-сервисами для размещения IT-проектов__*

Прежде всего, пользователям необходимо зарегистрироваться в веб-сервисе. После этого нужно перейти по адресу репозитория. Рассмотрим на примере репозитория [Андрея Булгакова](https://github.com/AndreyBulgakov19/SCV_Git_1804) в сервисе [Github](https://github.com/).

Чтобы начать работу с проектом, вам необходимо "форкнуть" его (кнопка в правом верхнем углу).

![вилка](fork.PNG)

Кнопка «Форк» создает копию оригинального репозитория в профиле пользователя. При разветвлении удаленного репо вы можете переименовать его по своему усмотрению.

![fork_creating](fork_creation.PNG)

Теперь вы можете клонировать его в локальный репозиторий, скопировав его URL-адрес и используя команду `git clone` в терминале редактора кода.

![git_clone](git_clone.PNG)

>__Nota bene! При работе в чужом проекте необходимо использовать другую ветку!__

### *__Создание pull request'a__*

После того, как проект будет готов, его необходимо отправить на рассмотрение. Для этого в первую очередь нужно использовать команду `git push`.

Как мы уже знаем, он используется для отправки всех обновленных коммитов из локальной ветки, которая хранится в локальной системе, в удаленную ветку.